import { PLUGIN_CREATE_ACTION_EVENT, PLUGIN_ENGINE_READY, PLUGIN_GET_APP_STYLE_URL_EVENT, PLUGIN_SUBSCRIBE_TO_EVENT, getActionEvent, } from './events';
...
export class PluginFrameEngine {
    ...
    createContext() {
        const ctx = {
            ...
            createAction: async (opts) => {
                ...
                const idPromise = this.worker.request(PLUGIN_CREATE_ACTION_EVENT, optsWithoutExecute);
    ...
    async handlePanelSetup(panelName) {
        ...
        if (panel) {
            ...
            panel.initialize(this.getContext(), await this.worker.request(PLUGIN_GET_APP_STYLE_URL_EVENT));
        }
    }
}
