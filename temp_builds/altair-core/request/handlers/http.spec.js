...
describe('HTTP handler', () => {
    it('should properly handle normal successful HTTP requests', async () => {
        ...
        expect(receivedRequest?.headers.get('X-GraphQL-Token')).toEqual('asd7-237s-2bdk-nsdk4');
    ...
    it('should properly handle aborting the request', () => {
        const mockHandler = new MswMockRequestHandler('http://localhost:3000/delay', async () => {
            await delay(1000);
            return Response.json({ data: { hello: 'world' } });
        });
        server.use(mockHandler);
    ...
    it('should properly handle streamed responses', async () => {
        const mockHandler = new MswMockRequestHandler('http://localhost:3000/simple-stream', async () => {
    ...
    it('should properly handle streamed responses with errors', async () => {
        const mockHandler = new MswMockRequestHandler('http://localhost:3000/error-stream', async () => {
            const encoder = new TextEncoder();
            const stream = new ReadableStream({
                async start(controller) {
                    await delay(10);
    ...
    it('should properly handle multipart streamed responses', async () => {
        const mockHandler = new MswMockRequestHandler('http://localhost:3000/multipart-stream', async () => {
            const encoder = new TextEncoder();
            const stream = new ReadableStream({
                async start(controller) {
                    await delay(10);
    ...
    it('should properly handle multipart streamed responses - sample 2', async () => {
        const mockHandler = new MswMockRequestHandler('http://localhost:3000/multipart-stream-2', async () => {
            const encoder = new TextEncoder();
            const stream = new ReadableStream({
                async start(controller) {
                    await delay(10);
    ...
    it('should properly handle multipart streamed responses with errors', async () => {
        const mockHandler = new MswMockRequestHandler('http://localhost:3000/error-multipart-stream', async () => {
            const encoder = new TextEncoder();
            const stream = new ReadableStream({
                async start(controller) {
                    await delay(10);
