...
export const buildResponse = (responses, strategy = MultiResponseStrategy.AUTO) => {
    switch (strategy) {
...
const buildResponse__patch = (responses) => {
    ...
    const obj = parseJson(responses[0]?.content ?? '', {
        defaultValue: null,
    });
    if (!obj) {
    ...
    for (let i = 1; i < responses.length; i++) {
        const nextObj = parseJson(responses[i]?.content ?? '', { defaultValue: null });
        if (!nextObj) {
            throw new Error('JSON response required for patching!');
        }
