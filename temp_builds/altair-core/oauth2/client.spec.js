...
describe('oauth2 client', () => {
    describe('getAuthorizationUrl', () => {
        it('should return the authorization url', async () => {
            ...
            expect(url).toBe('https://auth-server.com/oauth/authorize?response_type=code&client_id=clientId&redirect_uri=redirectUri&state=random-state&scope=scope1+scope2&code_challenge=N1E4yRMD7xixn_oFyO_W3htYN3rY7-HMDKJe6z6r928&code_challenge_method=S256');
        });
        it('should throw an error if the flow is client credentials', async () => {
            ...
            await expect(client.getAuthorizationUrl()).rejects.toThrow('Client Credentials flow not supported for authorization code');
        });
    });
    describe('getAuthorizationRedirectResponse', () => {
        ...
        it('should throw an error if the flow is client credentials', async () => {
            ...
            location.href =
                'https://auth-server.com/oauth/authorize?code=code&state=random-state';
            await expect(client.getAuthorizationRedirectResponse()).rejects.toThrow('Client Credentials flow not supported for authorization code');
        });
    });
    describe('getAccessTokenFromCode', () => {
        it('should return the access token from code (authFormat: in body, requestFormat: JSON)', async () => {
            ...
            const mockHandler = new MswMockRequestHandler('https://auth-server.com/oauth/token', async () => {
            ...
            expect(Object.fromEntries(receivedRequest?.headers)).toEqual({
        ...
        it('should return the access token from code (authFormat: in body, requestFormat: form)', async () => {
            ...
            expect(await receivedRequest?.text()).toEqual('grant_type=authorization_code&code=code&redirect_uri=redirectUri&client_id=clientId&client_secret=clientSecret&code_verifier=codeVerifier');
            expect(response).toEqual({
                access_token: 'access-token',
                refresh_token: 'refresh-token',
                expires_in: 3600,
        ...
        it('should return the access token from code (authFormat: basic auth, requestFormat: form)', async () => {
            ...
            expect(await receivedRequest?.text()).toEqual('grant_type=authorization_code&code=code&redirect_uri=redirectUri&code_verifier=codeVerifier');
            expect(response).toEqual({
                access_token: 'access-token',
                refresh_token: 'refresh-token',
                expires_in: 3600,
        ...
        it('should throw an error if the flow is client credentials', async () => {
            ...
            await expect(client.getAccessTokenFromCode('code')).rejects.toThrow('Client Credentials flow not supported');
        });
    });
    describe('getAccessTokenFromClientCredentials', () => {
        ...
        it('should return the access token from client credentials (authFormat: in body, requestFormat: form)', async () => {
            ...
            expect(await receivedRequest?.text()).toEqual('grant_type=client_credentials&client_id=clientId&client_secret=clientSecret&scope=scope1+scope2');
            expect(response).toEqual({
                access_token: 'access-token',
                refresh_token: 'refresh-token',
                expires_in: 3600,
        ...
        it('should return the access token from client credentials (authFormat: basic auth, requestFormat: form)', async () => {
            ...
            expect(await receivedRequest?.text()).toEqual('grant_type=client_credentials&scope=scope1+scope2');
            expect(response).toEqual({
                access_token: 'access-token',
                refresh_token: 'refresh-token',
                expires_in: 3600,
        ...
        it('should throw an error if the flow is not client credentials', async () => {
            ...
            await expect(client.getAccessTokenFromClientCredentials()).rejects.toThrow('Only Client Credentials flow is supported');
        });
    });
